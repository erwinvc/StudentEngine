...
private:
	...
	static GameObject* g_currentObject;
...
public:
	...
	static void CreateObject(GameObject* gameObject) {
		while (m_list.Size() > g_currentListIndex) {
			m_list.RemoveLast();
		}

		m_list.Add({ *gameObject, *gameObject, gameObject->m_id, false, true });
	...
	static void Record(GameObject* gameObject) {
		...
		m_recording = true;
		g_baseObject = gameObject->Copy();
		g_currentObject = gameObject;
	}

	static void FinishRecording(bool deleted) {
		if (!m_recording) return;
		if (deleted || !g_baseObject->Compare(g_currentObject)) {
			while (m_list.Size() > g_currentListIndex) {
				m_list.RemoveLast();
			}

			m_list.Add({ *g_baseObject, *g_currentObject, g_currentObject->m_id, deleted, false });
	...
	static void GoToNewListIndex() {
		while (g_newListIndex - g_currentListIndex < 0) { //Undo
			UndoEntryCollection& collection = m_list[g_currentListIndex - 1];
			if (collection.m_deleted) {
				GameObject* copy = collection.m_redoObject.Copy();
				GetEditorScene()->AddGameObject(copy);
			} else if (collection.m_created) {
		...
		while (g_newListIndex - g_currentListIndex > 0) { //Redo
			...
			if (collection.m_deleted) {
				...
				GameObject* copy = collection.m_baseObject.Copy();
				GetEditorScene()->AddGameObject(copy);
			} else {
				GameObject* obj = GetEditorScene()->FindObjectByID(collection.m_id);
				if (obj) *obj = collection.m_redoObject;
			}
			g_currentListIndex++;
		}
	}
